...
/**
 * ApiGetResource - Attribute para modelos que exponen API de solo lectura (GET)
 *
 * FILOSOFÍA LEGO:
 * "Table-Driven API" - Decorador específico para TableComponent.
 * Solo genera endpoints de LECTURA (GET), no CRUD completo.
 *
 * DIFERENCIA con ApiCrudResource:
 * - ApiGetResource: Solo GET (lectura para tablas) → /api/get/{resource}
 * - ApiCrudResource: CRUD completo (GET, POST, PUT, DELETE) → /api/{resource}
 *
 * ENDPOINTS GENERADOS:
 * - GET /api/get/products        → Listar con paginación/filtros
 ...
 *     // GET /api/get/products        (lista con paginación)
 *     // GET /api/get/products/{id}   (individual)
 * }
 * ```
 *
 * USO AVANZADO:
 ...
 * - Filtros y búsqueda
 * - Sin modificación de datos (solo lectura)
 */
#[Attribute(Attribute::TARGET_CLASS)]
class ApiGetResource
{
    /**
     * @param string $endpoint Endpoint sin prefijo /api/get (ej: 'products' o 'catalog/items')
    ...
    public function __construct(
        ...
        // Validar que endpoint NO incluya /api/get (se agrega automáticamente)
        if ($this->endpoint !== null &&
            (str_starts_with($this->endpoint, '/api/get') || str_starts_with($this->endpoint, '/api'))) {
            throw new \InvalidArgumentException(
                "Endpoint should not include '/api/get' or '/api' prefix. It's added automatically.\n" .
                "Instead of: '{$this->endpoint}'\n" .
                "Use: '" . preg_replace('#^/api(/get)?/?#', '', $this->endpoint) . "'"
            );
        }
     ...
     * por lo que Flight recibe URIs como /get/products, no /api/get/products.
     *
     * @param string $modelClass Nombre completo de la clase
     * @return string Endpoint para Flight (ej: '/get/products')
     */
    public function getEndpoint(string $modelClass): string
    {
        // Si endpoint está definido, agregar prefijo /get
        if ($this->endpoint !== null) {
